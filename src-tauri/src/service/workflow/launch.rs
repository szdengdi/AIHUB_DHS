...
pub async fn launch(app_handle: tauri::AppHandle) -> Result<(), String> {
    // AIHub 定制版：启动前幂等注入 CMI AI Hub 默认模型提供方
    // （settings.yaml 的 llm-pi-ai 段 + home 级 cordis.patch.yml 的
    // agent-default-model 覆盖；失败不阻断启动）。
    super::cmi_aihub::ensure_default_provider(&app_handle);
