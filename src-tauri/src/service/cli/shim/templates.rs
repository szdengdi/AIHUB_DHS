...
if (-not $node) {
    Write-Error 'Node.js runtime not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first.'
    exit 1
}
"#;
...
if [ -z "$NODE" ]; then
  echo "Node.js runtime not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first." >&2
  exit 1
fi
"#;
