...
const GENERATED_MARKER: &str = "DeepSeek Harness(AIHub定制版) Desktop - ";
...
/// 后的第二行）写 #/rem DeepSeek Harness(AIHub定制版) Desktop - ...；用户文件即使正文
...
mod tests {
    ...
    fn generated_phrase_outside_header_is_not_generated_shim() {
        ...
        // 前两行不是标记（用户脚本），正文（第三行）却提到"DeepSeek Harness(AIHub定制版) Desktop - "
        std::fs::write(
            &target,
            "@echo off\necho user shim\necho see DeepSeek Harness(AIHub定制版) Desktop - readme note here\n",
        )
        .unwrap();
        assert!(
    ...
    fn write_shim_file_overwrites_generated_shim() {
        ...
        std::fs::write(
            &target,
            "#!/bin/sh\n# DeepSeek Harness(AIHub定制版) Desktop - old shim\n",
        )
        .unwrap();

        write_shim_file(
            &target,
            "#!/bin/sh\n# DeepSeek Harness(AIHub定制版) Desktop - new shim\n",
        )
        .unwrap();
