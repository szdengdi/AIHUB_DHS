...
pub fn build_cmd_shim(app_dir: &Path, dsh_home: &Path) -> String {
    ...
    format!(
        r#"@echo off
rem DeepSeek Harness(AIHub定制版) Desktop - dsh command shim (generated)
...
:no_cli
echo [dsh] Harness CLI not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first. 1>&2
exit /b 1

:no_node
echo [dsh] Node.js runtime not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first. 1>&2
exit /b 1
"#,
...
pub fn build_ps1_shim(app_dir: &Path, dsh_home: &Path) -> String {
    ...
    format!(
        r#"# DeepSeek Harness(AIHub定制版) Desktop - dsh command shim (generated)
...
if (-not (Test-Path -LiteralPath $dshBin)) {{
    Write-Error '[dsh] Harness CLI not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first.'
    exit 1
}}
...
{node_resolve}
if [ ! -f "$DSH_BIN" ]; then
  echo "[dsh] Harness CLI not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first." >&2
  exit 1
fi
exec "$NODE" "$DSH_BIN" "$@"
"#,
...
pub fn build_pnpm_cmd_shim(app_dir: &Path) -> String {
    ...
    format!(
        r#"@echo off
rem DeepSeek Harness(AIHub定制版) Desktop - pnpm command shim (generated)
rem Do not edit: regenerated by the desktop app on install/startup.
setlocal
set "APP_DIR={app_dir}"
...
:no_pnpm
echo [pnpm] pnpm not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first. 1>&2
exit /b 1

:no_node
echo [pnpm] Node.js runtime not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first. 1>&2
exit /b 1
"#,
        app_dir = escape_path_cmd(app_dir),
...
pub fn build_pnpm_ps1_shim(app_dir: &Path) -> String {
    ...
    format!(
        r#"# DeepSeek Harness(AIHub定制版) Desktop - pnpm command shim (generated)
# Do not edit: regenerated by the desktop app on install/startup.
$ErrorActionPreference = "Stop"
$appDir = '{app_dir}'
$pnpmBin = '{pnpm_bin}'
...
if (-not (Test-Path -LiteralPath $pnpmBin)) {{
    Write-Error '[pnpm] pnpm not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first.'
    exit 1
}}
& $node $pnpmBin @args
exit $LASTEXITCODE
...
if [ ! -f "$PNPM_BIN" ]; then
  echo "[pnpm] pnpm not found. Please run DeepSeek Harness(AIHub定制版) Desktop to install it first." >&2
  exit 1
fi
exec "$NODE" "$PNPM_BIN" "$@"
"#,
