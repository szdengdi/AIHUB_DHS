...
pub fn t(key: &str) -> String {
    let (zh, en): (&str, &str) = match key {
        ...
        "harness.title" => ("DeepSeek Harness(AIHub定制版) 核心", "DeepSeek Harness(AIHub定制版) core"),
        "harness.core_not_found" => (
            "未找到 DeepSeek Harness(AIHub定制版) 核心包，请先完成安装",
            "DeepSeek Harness(AIHub定制版) core package not found, run setup first",
        ),
        ...
        "harness.start_failed" => (
            "启动 DeepSeek Harness(AIHub定制版) 服务失败",
            "Failed to start DeepSeek Harness(AIHub定制版) service",
        ),
        "harness.health_unhealthy" => (
            "DeepSeek Harness(AIHub定制版) 服务未就绪",
            "DeepSeek Harness(AIHub定制版) service is not ready",
        ),
